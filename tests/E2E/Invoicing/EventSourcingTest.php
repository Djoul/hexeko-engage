...
/**
 * E2E-004: Division Balance Event Sourcing
 *
 * Validates projection of division balance from stored events
 *
 * Test Scenario:
 * - Generate 3 division invoices (€1000, €1500, €2000)
 * - Pay first 2 invoices
 * - Delete projection and replay events
 *
 * Critical Validations:
 * ✅ 5 events recorded (3 generated + 2 paid)
 * ✅ Correct event types (DivisionInvoiceGenerated, DivisionInvoicePaid)
 * ✅ Balance = €2000 (€4500 generated - €2500 paid)
 * ✅ Replay events reconstructs identical balance
 * ✅ last_invoice_at updated correctly
 * ✅ Aggregate UUID = division_id
 */
#[Group('e2e')]
#[Group('e2e-critical')]
#[Group('invoicing')]
#[Group('event-sourcing')]
class EventSourcingTest extends E2ETestCase
{
    #[Test]
    public function e2e_004_it_projects_division_balance_from_events(): void
    {
        // ============================================================
        ...
        Event::fake([
            DivisionInvoiceGenerated::class,
            DivisionInvoicePaid::class,
        ]);

        ['division' => $division] = $this->createTestFinancerWithDivision([
            'name' => 'Event Sourced Division',
            'country' => 'BE',
            'vat_rate' => 21.00,
            'core_package_price' => 100000,
        ]);

        // Create 3 division invoices with different amounts
        ...
        foreach ($invoiceAmounts as $index => $amount) {
            $invoice = Invoice::factory()->create([
                ...
                'vat_amount' => (int) round($amount * 0.21),
                'total_ttc' => $amount + (int) round($amount * 0.21),
                'confirmed_at' => now(),
            ]);
            ...
            Event::dispatch(new DivisionInvoiceGenerated($division->id, $invoice->id, $invoice->total_ttc, now()));
        }
        ...
        foreach (array_slice($invoices, 0, 2) as $invoice) {
            ...
            Event::dispatch(new DivisionInvoicePaid($division->id, $invoice->id, $invoice->total_ttc, now()));
        }
        ...
        foreach ($invoices as $index => $invoice) {
            DB::table('stored_events')->insert([
                ...
                'event_version' => 1,
                'event_class' => DivisionInvoiceGenerated::class,
        ...
        foreach (array_slice($invoices, 0, 2) as $index => $invoice) {
            DB::table('stored_events')->insert([
                ...
                'event_version' => 1,
                'event_class' => DivisionInvoicePaid::class,
                'event_properties' => json_encode([
                    'divisionId' => $division->id,
                    'invoiceId' => $invoice->id,
                    'amount' => $invoice->total_ttc,
        ...
        $generatedEvents = $storedEvents->where('event_class', DivisionInvoiceGenerated::class);
        $paidEvents = $storedEvents->where('event_class', DivisionInvoicePaid::class);

        $this->assertCount(3, $generatedEvents, 'Should have 3 DivisionInvoiceGenerated events');
        $this->assertCount(2, $paidEvents, 'Should have 2 DivisionInvoicePaid events');

        // ✅ 3. Verify balance calculation
        // Generated: €1000 + €1500 + €2000 = €4500 (with VAT: €5445)
        // Paid: €1000 + €1500 = €2500 (with VAT: €3025)
        // Balance: €5445 - €3025 = €2420
        $expectedBalance = $generatedTotal - $paidTotal;
        $this->assertEquals($expectedBalance, $balance->balance);

        // ✅ 4. Verify aggregate UUID
        $this->assertEquals($division->id, $aggregateUuid);
        ...
        $replayedBalance = DivisionBalance::firstOrCreate([
            'division_id' => $division->id,
        ]);
        ...
        foreach ($storedEvents as $event) {
            ...
            if ($event->event_class === DivisionInvoiceGenerated::class) {
                $replayGeneratedTotal += $properties['amount'];
            } elseif ($event->event_class === DivisionInvoicePaid::class) {
        ...
        $actualInvoicedTotal = Invoice::where('recipient_type', 'App\\Models\\Division')
            ->where('recipient_id', $division->id)
            ->where('status', '!=', 'draft')
            ->sum('total_ttc');

        $actualPaidTotal = Invoice::where('recipient_type', 'App\\Models\\Division')
            ->where('recipient_id', $division->id)
            ->where('status', 'paid')
            ->sum('total_ttc');
    ...
    #[Test]
    public function e2e_004b_it_handles_concurrent_event_recording(): void
    {
        // ============================================================
    ...
    #[Test]
    public function e2e_004c_it_tracks_balance_changes_over_time(): void
    {
        // ============================================================
        ...
        for ($month = 1; $month <= 5; $month++) {
            ...
            $cumulativeBalance = Invoice::where('recipient_type', 'App\\Models\\Division')
                ->where('recipient_id', $division->id)
                ->sum('total_ttc');
        ...
        $balance = DivisionBalance::firstOrCreate(['division_id' => $division->id]);
